...
public class ArithmeticLexer {
    public enum TokenType {
        NUM, POW, MULTIPLY, PLUS, MINUS, LB, RB, 
        EOF
    }
    ...
    private void init() {
        ...
        matchers.add(new TokenMatcher(TokenType.NUM, "[0-9]+", true));
        matchers.add(new TokenMatcher(TokenType.POW, "\\^|\\*\\*", true));
        matchers.add(new TokenMatcher(TokenType.MULTIPLY, "*", false));
    ...
    public ArithmeticLexer(String s) throws ParseException {
        input = s;
        init();
        nextChar();
    }
